...
def get_services():
    """Liste toutes les prestations, avec leurs déclinaisons et options."""
    services = Service.query.all()
    return jsonify([s.to_dict(include_details=True) for s in services])
...
def create_service():
    ...
    service = Service(
        ...
        deposit_is_percent=bool(data.get("deposit_is_percent", False)),
    ...
    db.session.commit()
    return jsonify({"message": "Prestation créée", "service": service.to_dict(include_details=True)}), 201


@admin_bp.route("/services/<int:service_id>", methods=["PUT"])
def update_service(service_id):
    """Modifier une prestation (infos, détails, prix)."""
    ...
    if "deposit_amount" in data:
        service.deposit_amount = float(data["deposit_amount"])
    if "deposit_is_percent" in data:
        service.deposit_is_percent = bool(data["deposit_is_percent"])
    if "requires_wig_deposit" in data:
        service.requires_wig_deposit = bool(data["requires_wig_deposit"])
    if "is_pack" in data:
        service.is_pack = bool(data["is_pack"])
    if "is_active" in data:
        service.is_active = bool(data["is_active"])

    db.session.commit()
    return jsonify({"message": "Prestation mise à jour", "service": service.to_dict(include_details=True)})


# ─────────────────────────────────────────
# SERVICE VARIANTS (déclinaisons des packs)
# ─────────────────────────────────────────
@admin_bp.route("/services/<int:service_id>/variants", methods=["POST"])
def create_service_variant(service_id):
    """Ajouter une déclinaison (longueur/style + prix) à un pack."""
    service = Service.query.get_or_404(service_id)
    data = request.json or {}
    required = ["label", "price"]
    if not all(f in data for f in required):
        return jsonify({"error": "Champs requis: label, price"}), 400
    ...
    db.session.add(variant)
    db.session.commit()
    return jsonify({"message": "Déclinaison créée", "variant": variant.to_dict()}), 201


@admin_bp.route("/services/variants/<int:variant_id>", methods=["PUT"])
def update_service_variant(variant_id):
    """Modifier le prix/libellé d'une déclinaison."""
    variant = ServiceVariant.query.get_or_404(variant_id)
    data = request.json or {}

    for field in ["label", "length", "style"]:
        if field in data:
    ...
    db.session.commit()
    return jsonify({"message": "Déclinaison mise à jour", "variant": variant.to_dict()})


@admin_bp.route("/services/variants/<int:variant_id>", methods=["DELETE"])
def delete_service_variant(variant_id):
    """Désactiver une déclinaison."""
    variant = ServiceVariant.query.get_or_404(variant_id)
    variant.is_active = False
    db.session.commit()
    return jsonify({"message": "Déclinaison désactivée"})


# ─────────────────────────────────────────
# SERVICE OPTIONS (ex: coloration)
# ─────────────────────────────────────────
@admin_bp.route("/services/<int:service_id>/options", methods=["POST"])
def create_service_option(service_id):
    """Ajouter une option payante (ex: coloration) à une prestation."""
    service = Service.query.get_or_404(service_id)
    data = request.json or {}
    required = ["name", "price"]
    if not all(f in data for f in required):
        return jsonify({"error": "Champs requis: name, price"}), 400

    option = ServiceOption(
        service_id=service.id,
        name=data["name"],
        price=float(data["price"]),
        allows_photo=bool(data.get("allows_photo", False)),
    )
    db.session.add(option)
    db.session.commit()
    return jsonify({"message": "Option créée", "option": option.to_dict()}), 201


@admin_bp.route("/services/options/<int:option_id>", methods=["PUT"])
def update_service_option(option_id):
    """Modifier une option payante."""
    ...
    if "allows_photo" in data:
        option.allows_photo = bool(data["allows_photo"])
    if "is_active" in data:
        option.is_active = bool(data["is_active"])

    db.session.commit()
    return jsonify({"message": "Option mise à jour", "option": option.to_dict()})


@admin_bp.route("/services/options/<int:option_id>", methods=["DELETE"])
def delete_service_option(option_id):
    """Désactiver une option payante."""
    option = ServiceOption.query.get_or_404(option_id)
    option.is_active = False
    db.session.commit()
    return jsonify({"message": "Option désactivée"})
